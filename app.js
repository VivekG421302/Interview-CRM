...
function applyAccent(hex){
  ...
  // Write directly to the vars CSS reads — no chained var() indirection
  root.style.setProperty('--accent',     p.accent);
  root.style.setProperty('--accent-dim', p.accentDim);
  root.style.setProperty('--accent-mid', p.accentMid);
  root.style.setProperty('--accent-on',  p.accentOn);
  root.style.setProperty('--accent-2',   p.accent2);
  root.style.setProperty('--accent-3',   p.accent3);
  // Update PWA theme-color meta tag
  const meta = document.querySelector('meta[name="theme-color"]');
  if(meta) meta.setAttribute('content', hex);
  // Redraw charts so they pick up the new palette
